...
class HiAlDataset(BaseDataset):

    def __init__(self, split):
        super().__init__()
        # Split can be test, val, or ltrval (a validation split consisting of videos from the official train set)
        if split == 'HiAl' or split == 'val':
            self.base_path = os.path.join(self.env_settings.HiAl_path, split)
        else:
            self.base_path = os.path.join(self.env_settings.HiAl_path, 'train')
    ...
    def _construct_sequence(self, sequence_name):
        ...
        frames_path_i = os.path.join(self.base_path, sequence_name, 'infrared')
        frames_path_v = os.path.join(self.base_path, sequence_name, 'visible')
        ...
        return Sequence(sequence_name, frames_list, 'HiAl', ground_truth_rect.reshape(-1, 4))
