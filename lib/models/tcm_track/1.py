...
from lib.models.layers.tcm_layer import TBSILayer
...
class VisionTransformerTCM(BaseBackbone):
    """ Vision Transformer
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 tcm_loc=None, tcm_drop_path=None):
        """
        Args:
        ...
        self.tcm_loc = tbsi_loc
        self.tcm_drop_path = tbsi_drop_path
        self.tcm_layers = nn.ModuleList()
        if self.tbsi_loc is not None and type(self.tbsi_loc) == list:
            for i in range(len(self.tbsi_loc)):
                self.tcm_layers.append(
                    TBSILayer(dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias,
                              drop=drop_rate,
    ...
    def forward_features(self, z, x, x_i1=None, x_v1=None, z_i1=None, z_v1=None):
        ...
        if z_v1 is None:
            z_v1 = torch.zeros_like(z_v)
        if x_v1 is None:
            x_v1 = torch.zeros_like(x_v)
        if z_i1 is None:
            z_i1 = torch.zeros_like(z_i)
        if x_i1 is None:
            x_i1 = torch.zeros_like(x_i)
        ...
        # Update the previous frame variables for the next call
        z_v1 = z_v
        x_v1 = x_v
        z_i1 = z_i
        x_i1 = x_i
...
@torch.no_grad()
def _load_weights(model: VisionTransformerTCM, checkpoint_path: str, prefix: str = ''):
...
def _create_vision_transformer(variant, pretrained=False, default_cfg=None, **kwargs):
    ...
    model = VisionTransformerTCM(**kwargs)
...
def vit_base_patch16_224_tcm(pretrained: object = False, **kwargs: object) -> object:
    """
...
def vit_small_patch16_224_tcm(pretrained=False, **kwargs):
    """
...
def vit_tiny_patch16_224_tcm(pretrained=False, **kwargs):
    """
