...
from lib.models.layers.tcm_layer import TCMLayer
...
class VisionTransformerTCM(BaseBackbone):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        if self.tcm_loc is not None and type(self.tbsi_loc) == list:
            for i in range(len(self.tbsi_loc)):
                self.tcm_layers.append(
                    TCMLayer(dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias,
                              drop=drop_rate,
...
def vit_base_patch16_224_tcm(pretrained=False, **kwargs):
    """
