...
class TCMLayer(nn.Module):
    ...
    def forward(self, x_v, x_i, lens_z):
        ...
        fused_ti = self.ca_s2t_i2f(torch.cat([fused_t, x_i[:, lens_z:, :]], dim=1))[:, :lens_z, :]
        temp_x_v = self.ca_t2s_f2v(torch.cat([fused_ti, x_v[:, lens_z:, :]], dim=1))[:, lens_z:, :]
        fused_tv = self.ca_s2t_v2f(torch.cat([fused_t, x_v[:, lens_z:, :]], dim=1))[:, :lens_z, :]
        temp_x_i = self.ca_t2s_f2i(torch.cat([fused_tv, x_i[:, lens_z:, :]], dim=1))[:, lens_z:, :]

        fused_t = self.t_fusion(torch.cat([fused_ti[:, :lens_z, :], fused_tv[:, :lens_z, :]], dim=2))

        v = self.t_fusion(torch.cat([fused_tv[:, :lens_z, :], fused_t], dim=2))
        i = self.t_fusion(torch.cat([fused_tv[:, :lens_z, :], fused_t], dim=2))

        x_v[:, lens_z:, :] = temp_x_v
        x_i[:, lens_z:, :] = temp_x_i

        x_v[:, :lens_z, :] = self.ca_t2t_f2v(torch.cat([x_v[:, :lens_z, :], v], dim=1))[:, :lens_z, :]
        x_i[:, :lens_z, :] = self.ca_t2t_f2i(torch.cat([x_i[:, :lens_z, :], i], dim=1))[:, :lens_z, :]
